...
parser = argparse.ArgumentParser(
    description="Train a shared-policy PPO controller for multiple student vehicles inside each environment."
)
parser.add_argument("--num_envs", type=int, default=64, help="Number of parallel world instances.")
parser.add_argument("--num_agents_per_env", type=int, default=2, help="Number of vehicles inside each world.")
parser.add_argument("--seed", type=int, default=42, help="Random seed.")
parser.add_argument("--total_timesteps", type=int, default=3_000_000, help="Total PPO rollout slots to train for.")
parser.add_argument("--log_dir", type=str, default="logs/sb3/student_vehicle_goal_multiagent", help="Training log root.")
...
parser.add_argument("--goal_radius_min_m", type=float, default=5.0, help="Minimum goal radius from env origin.")
parser.add_argument("--goal_radius_max_m", type=float, default=8.0, help="Maximum goal radius from env origin.")
parser.add_argument("--n_steps", type=int, default=64, help="SB3 PPO rollout horizon per rollout slot.")
parser.add_argument("--batch_size", type=int, default=4096, help="SB3 PPO minibatch size.")
...
parser.add_argument(
    ...
    type=int,
    default=250_000,
    help="Save a checkpoint every N rollout-slot timesteps.",
)
parser.add_argument(
    "--normalize_obs",
    action="store_true",
...
from src.sb3_shared_multiagent_vec_env import Sb3SharedMultiAgentVecEnvWrapper
...
def _write_run_metadata(run_dir: Path, env_cfg: StudentVehicleMultiAgentGoalEnvCfg, env: StudentVehicleMultiAgentGoalEnv):
...
def main():
    ...
    checkpoint_freq = max(1, int(args_cli.checkpoint_every) // max(1, int(vec_env.num_envs)))
    callbacks = [
    ...
    try:
        ...
        vec_env.close()
        print(f"[INFO] Multi-agent training finished in {time.time() - start_time:.2f}s")
